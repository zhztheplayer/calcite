...
public class SqlFunction extends SqlOperator {
  ...
  /**
   * Throws a validation error if a WITHIN GROUP clause is present but
   * not allowed.
   */
  private void validateCollation(SqlValidator validator, SqlCall call) {
    if (!SqlNodeList.isEmptyList(call.getAggOrderList()) && !allowsOrderedAggregate()) {
