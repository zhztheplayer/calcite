...
public abstract class SqlAggFunction extends SqlFunction implements Context {
  ...
  /**
   * Returns whether aggregate order ({@code WITHIN GROUP}) should be ignored. E.g.
   * {@code SUM(X) WITHIN GROUP (ORDER BY ...)} is equivalent to {@code SUM(X)} so that the
   * {@code WITHIN GROUP} clause should be ignored.
   *
