...
public class EnumerableAggregate extends Aggregate implements EnumerableRel {
  ...
  public Result implement(EnumerableRelImplementor implementor, Prefer pref) {
    ...
    boolean hasOrderedCall = false;
    for (AggImpState agg : aggs) {
      if (!agg.call.getOrdering().equals(RelCollations.EMPTY)) {
        hasOrderedCall = true;
        break;
      }
    }

    ParameterExpression lambdaFactory = Expressions.parameter(
        AggregateLambdaFactory.class,
        builder.newName("lambdaFactory")
    );

    implementLambdaFactory(builder, inputPhysType, aggs, accumulatorInitializer, hasOrderedCall,
        lambdaFactory);
    // Function2<Integer, Object[], Object[]> resultSelector =
    //     new Function2<Integer, Object[], Object[]>() {
  ...
  /**
   * Implement the {@link AggregateLambdaFactory}.
   *
   * 1. hasOrderedCall == true
   * Means there is at least one aggregate call including sort spec. We use
   * {@link OrderedAggregateLambdaFactory} implementation to implement
   * sorted aggregates for that.
   *
   * 2. hasOrderedCall == false
   * Use {@link SequencedAdderAggregateLambdaFactory} to implement a non-sort aggregate.
   */
    ...
    if (hasOrderedCall) {
      ...
      builder.add(
          Expressions.declare(
              0,
              pe,
              Expressions.new_(LinkedList.class)
          ));

      for (AggImpState agg : aggs) {
        RelCollation relCollation = agg.call.getOrdering();
        if (relCollation.equals(RelCollations.EMPTY)) {
          continue;
        }
